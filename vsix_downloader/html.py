...
def _iter_meta_data_json(download_dir: Path, is_flatten: bool) -> Generator[Path, Any, None]:
...
async def _iter_meta_data(download_dir: Path, is_flatten: bool) -> AsyncGenerator[VSCodeExtension, Any]:
    for meta_path in _iter_meta_data_json(download_dir, is_flatten):
...
async def _load_extensions_render_params(download_dir: Path, is_flatten: bool) -> list[dict[str, Any]]:
    results: list = []
    async for ext_meta_data in _iter_meta_data(download_dir, is_flatten):
        ...
        for ext_version in ext_meta_data.versions:
            ...
            if is_flatten:
                ...
                file_path = download_dir / ext_meta_data.unified_name / download_file_name
            ...
            if file_path.is_file():
                versions.append({
                    ...
                    "last_updated": ext_version.last_updated.strftime("%Y-%m-%d %H:%M:%S"),
                    "code_engine": ext_version.code_engine,
        ...
        results.append({
            ...
            "extension_name": ext_meta_data.unified_name,
